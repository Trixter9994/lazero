...
void child_process(FILE *log_fp, struct config *_config) {
    ...
    if (_config->seccomp_rule_name != NULL) {
        if (strcmp("c_cpp", _config->seccomp_rule_name) == 0) {
     	...
     	else if (strcmp("random", _config->seccomp_rule_name) == 0) {
            if (random_seccomp_rules(_config) != SUCCESS ) {
                CHILD_ERROR_EXIT(LOAD_SECCOMP_FAILED);
            }
        }
	else if (strcmp("randomfuck", _config->seccomp_rule_name) == 0) {
            if (randomfuck_seccomp_rules(_config) != SUCCESS ) {
                CHILD_ERROR_EXIT(LOAD_SECCOMP_FAILED);
            }
        }
	// why bother?
